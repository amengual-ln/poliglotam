...
<section>
  ...
  <h3 class="mt-2">{@html texts[selectedLang].communication.title}</h3>
  {#each Object.entries(texts[selectedLang].communication.items) as [key, item]}
    <p>{item}</p>
  {/each}

  <h3 class="mt-4">{@html texts[selectedLang].confirmation.title}</h3>
  {#each Object.entries(texts[selectedLang].confirmation.items) as [key, item]}
    <p>{item}</p>
  {/each}
